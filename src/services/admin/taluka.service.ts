...
export const addTaluka = async (params: object) => {
    try {
        let sql = `SELECT DISTRICT_ID FROM taluka WHERE DISTRICT_ID = ? AND RTRIM(TALUKA_NAME)=? AND DELETED_AT IS NULL`;
    ...
    try {
        ...
        return executeQuery(sql, params).then(result => {
            if (result && (result as any[]).length > 0) {
                ...
                let sql = `UPDATE taluka SET DISTRICT_ID = ?, TALUKA_NAME=? WHERE TALUKA_ID = ? and DELETED_AT IS NULL`
                return executeQuery(sql, params).then(result => {
                    return (result) ? result : null;
                }).catch(error => {
...
export const deleteTaluka = async (params: object) => {
    try {
        let sql = `UPDATE taluka SET DELETED_AT = NOW() WHERE TALUKA_ID = ?`
        return executeQuery(sql, params).then(result => {
            return (result) ? result : null;
        }).catch(error => {
...
export const getTalukaListBYDistrictID = async (params: object) => {
    try {
        let sql = `select TALUKA_ID,TALUKA_NAME from taluka t where DISTRICT_ID = ? and DELETED_AT IS NULL order by TALUKA_ID desc`;
        return executeQuery(sql, [params]).then(result => {
            return (result) ? result : null;
        }).catch(error => {
            console.error("getTalukaList fetch data error: ", error);
