...
export const getGatGramPanchayat = async (params: object) => {
    try {
        let sql = `select g.GATGRAMPANCHAYAT_ID ,g.DISTRICT_ID ,g.TALUKA_ID ,g.PANCHAYAT_ID ,RTRIM(g.GATGRAMPANCHAYAT_NAME) as GATGRAMPANCHAYAT_NAME ,RTRIM(d.DISTRICT_NAME) as DISTRICT_NAME,RTRIM(t.TALUKA_NAME) as TALUKA_NAME,RTRIM(p.PANCHAYAT_NAME) as PANCHAYAT_NAME from gatgrampanchayat g INNER JOIN  district d on g.DISTRICT_ID  = d.DISTRICT_ID INNER JOIN taluka t  on g.TALUKA_ID  = t.TALUKA_ID INNER JOIN panchayat p on g.PANCHAYAT_ID =p.PANCHAYAT_ID where g.GATGRAMPANCHAYAT_ID =? and g.IS_DELETE =0`
        return executeQuery(sql, params).then(result => {
