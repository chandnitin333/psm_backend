...
export const getKaryaKarniCommiteeList = async (offset: number, search: string) => {
    try {
        let query = 'SELECT * FROM designation WHERE IS_DELETE=0';
        const params: any[] = [];
...
export const getKaryaKarniCommiteeById = async (id: number) => {
    try {
        const result = await executeQuery('SELECT * FROM designation WHERE DESIGNATION_ID = ? AND IS_DELETE=0', [id]);
        return result[0];
    } catch (err) {
...
export const softDeleteKaryaKarniCommitee = async (id: number) => {
    try {
        return await executeQuery('UPDATE designation SET IS_DELETE=1 WHERE DESIGNATION_ID = ?', [id]);
...
export const getTotalKaryaKarniCommiteeCount = async (search='') => {
    try {
        let result: any;
        if(search) {
            result = await executeQuery('SELECT COUNT(*) AS total FROM designation WHERE LOWER(DESIGNATION_NAME) LIKE LOWER(?) AND IS_DELETE=0', [`%${search}%`]);
        }else{
            result = await executeQuery('SELECT COUNT(*) AS total FROM designation WHERE IS_DELETE = 0', []);
        }
