...
export const getDistrict = async (params: object) => {
    try {
        let sql = `SELECT DISTRICT_ID,RTRIM(DISTRICT_NAME) FROM district WHERE DISTRICT_ID = ? AND IS_DELETE = 0`
        return executeQuery(sql, params).then(result => {
...
export const getDistrictList = async (params: object) => {
    try {
        ...
        let sql = `SELECT DISTRICT_ID, RTRIM(DISTRICT_NAME) FROM district WHERE IS_DELETE = 0 ORDER BY DISTRICT_ID DESC LIMIT ? OFFSET ?`;
